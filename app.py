...
""".format(
    ...
    uploaded_files = st.file_uploader(
        "Upload Curves, Levels, Source XML files OR WRPM file",
        type=["xml", "wrpm"],
        accept_multiple_files=True,
        key=f"file_uploader_{st.session_state.file_uploader_key}",
        help="Upload exactly 3 XML files (Curves, Levels, Source) OR 1 WRPM file"
    )
    ...
    if uploaded_files:
        # Detect file format (WRPM or XML)
        first_file = uploaded_files[0]
        is_wrpm = first_file.name.lower().endswith('.wrpm')

        if is_wrpm:
            # WRPM format - expect single file
            if len(uploaded_files) != 1:
                st.error(f"❌ Please upload exactly 1 WRPM file. You uploaded {len(uploaded_files)} files.")
                st.info("💡 Upload a single .wrpm file containing compressor diagnostic data")
                return None
            ...
            try:
                ...
                # Load WRPM data
                status_text.text("📄 Extracting waveform data...")
                progress_bar.progress(50)

                curves_df, levels_dict, source_dict = load_wrpm_file(first_file)

                # Create files_content structure compatible with XML flow
                files_content = {
                    'wrpm_mode': True,
                    'curves_df': curves_df,  # Direct DataFrame, no XML parsing needed
                    'levels_dict': levels_dict,
                    'source_dict': source_dict,
                    'curves': None,  # No XML content for WRPM
                    'levels': None,
                    'source': None
                }
                ...
                # Create preview info for WRPM
                preview_info = {
                    'machine_id': levels_dict.get('machine_id', 'Unknown'),
                    'rpm': levels_dict.get('rpm', 'Not available - manual input required'),
                    'date_time': levels_dict.get('date', 'Unknown'),
                    'cylinder_count': levels_dict.get('cylinder_count', 0),
                    'total_curves': len([col for col in curves_df.columns if col != 'Crank Angle']),
                    'file_sizes': {'wrpm': first_file.size / 1024}
                }
                ...
                progress_bar.progress(100)
                status_text.text("❌ WRPM parsing failed!")
                st.error(f"❌ Failed to parse WRPM file: {e}")
                st.info("💡 Please ensure you uploaded a valid .wrpm file from Windrock analyzer")
                return None

        else:
            # XML format - expect 3 files
            if len(uploaded_files) != 3:
...
if validated_files:
    ...
    is_wrpm_mode = files_content.get('wrpm_mode', False)

    # Check if we have required data (either XML or WRPM format)
    has_required_data = (
        is_wrpm_mode or
        ('curves' in files_content and 'source' in files_content and 'levels' in files_content)
    )
    ...
    if has_required_data:
        ...
        if st.session_state.analysis_results is None:
            with st.spinner("🔄 Processing data..."):
                if is_wrpm_mode:
                    # WRPM mode: use pre-parsed DataFrame
                    df = files_content['curves_df']
                    actual_curve_names = [col for col in df.columns if col != 'Crank Angle']

                    # Create minimal config for WRPM files
                    levels_dict = files_content['levels_dict']
                    source_dict = files_content['source_dict']

                    # Build discovered_config manually from WRPM data
                    discovered_config = {
                        'machine_id': levels_dict.get('machine_id', 'Unknown'),
                        'rated_rpm': levels_dict.get('rpm', 'N/A'),
                        'cylinders': []
                    }

                    # Auto-discover cylinders from column names
                    cylinder_map = {}
                    for col in actual_curve_names:
                        # Extract cylinder number from column name (e.g., "Comp 1 H Pressure" -> 1)
                        import re
                        match = re.search(r'Comp (\d+)', col)
                        if match:
                            cyl_num = int(match.group(1))
                            if cyl_num not in cylinder_map:
                                cylinder_map[cyl_num] = {
                                    'cylinder_name': f'Cylinder {cyl_num}',
                                    'pressure_curves': [],
                                    'valve_vibration_curves': []
                                }
                            # Determine if it's pressure or vibration curve
                            if 'Pressure' in col:
                                cylinder_map[cyl_num]['pressure_curves'].append(col)

                    discovered_config['cylinders'] = list(cylinder_map.values())

                else:
                    # XML mode: parse XML content
                ...
                # Common processing for both XML and WRPM modes
