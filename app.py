...
def calculate_real_theoretical_pressure(
    crank_angle_degrees: np.ndarray,
    ...
    Args:
        crank_angle_degrees: Array of crank angles (0-720 degrees for 4-stroke)
        compression_ratio: Actual compression ratio from XML data
        expansion_n: Polytropic expansion exponent from XML
        compression_n: Polytropic compression exponent from XML  
        suction_pressure: Suction pressure (PSI)
        discharge_pressure: Discharge pressure (PSI)
        clearance_pct: Clearance volume as % of swept volume
        bore: Cylinder bore (inches) - optional for volume calculations
        stroke: Stroke length (inches) - optional for volume calculations
    
    Returns:
        Dictionary containing theoretical pressure curves and cycle info
    """
    
    # Convert to radians for calculations
    theta_rad = np.deg2rad(crank_angle_degrees)
    
    # Calculate clearance volume ratio
    clearance_ratio = clearance_pct / 100.0
    
    # Calculate volume ratio at each crank angle (simplified piston motion)
    # V = Vc + (Vs/2) * (1 - cos(θ)) where Vc=clearance, Vs=swept volume
    volume_ratio = clearance_ratio + 0.5 * (1 - np.cos(theta_rad))
    
    # Normalize to get relative volume (1.0 = minimum volume at TDC)
    min_volume_ratio = clearance_ratio
    max_volume_ratio = clearance_ratio + 1.0
    normalized_volume = (volume_ratio - min_volume_ratio) / (max_volume_ratio - min_volume_ratio)
    
    # Initialize pressure array
    theoretical_pressure = np.zeros_like(crank_angle_degrees, dtype=float)
    
    # Define cycle phases based on crank angle (simplified 4-stroke equivalent)
    for i, angle in enumerate(crank_angle_degrees):
        angle_mod = angle % 720  # Full cycle
        
        if 0 <= angle_mod <= 180:  # Compression stroke
            # Polytropic compression: P₁V₁ⁿ = P₂V₂ⁿ
            volume_at_angle = max_volume_ratio - (max_volume_ratio - min_volume_ratio) * (angle_mod / 180)
            volume_at_angle = max(volume_at_angle, min_volume_ratio)  # Prevent division by zero
            
            # P = P₁ * (V₁/V₂)ⁿ
            pressure_ratio = (max_volume_ratio / volume_at_angle) ** compression_n
            theoretical_pressure[i] = suction_pressure * pressure_ratio
            
        elif 180 < angle_mod <= 360:  # Power/Expansion stroke
            # Polytropic expansion from peak pressure
            peak_pressure = suction_pressure * (compression_ratio ** compression_n)
            
            # Calculate volume during expansion
            volume_at_angle = min_volume_ratio + (max_volume_ratio - min_volume_ratio) * ((angle_mod - 180) / 180)
            
            # P = P₁ * (V₁/V₂)ⁿ for expansion
            pressure_ratio = (min_volume_ratio / volume_at_angle) ** expansion_n
            theoretical_pressure[i] = peak_pressure * pressure_ratio
            
        elif 360 < angle_mod <= 540:  # Exhaust stroke
            # Approximately constant at discharge pressure with some variation
            exhaust_variation = 1.0 + 0.05 * np.sin(np.deg2rad(angle_mod - 360))
            theoretical_pressure[i] = discharge_pressure * exhaust_variation
            
        else:  # Intake stroke (540-720)
            # Approximately constant at suction pressure with some variation
            intake_variation = 1.0 + 0.03 * np.sin(np.deg2rad(angle_mod - 540))
            theoretical_pressure[i] = suction_pressure * intake_variation
    
    # Calculate additional cycle information
    cycle_info = {
        'peak_pressure': np.max(theoretical_pressure),
        'compression_work': calculate_compression_work(theoretical_pressure, volume_ratio, suction_pressure),
        'expansion_work': calculate_expansion_work(theoretical_pressure, volume_ratio, discharge_pressure),
        'indicated_power': estimate_indicated_power(theoretical_pressure, volume_ratio, bore, stroke),
        'volumetric_efficiency': estimate_volumetric_efficiency(compression_ratio, clearance_ratio),
        'compression_ratio_actual': np.max(theoretical_pressure) / suction_pressure
    }
    
    return {
        'pressure': theoretical_pressure,
        'volume_ratio': volume_ratio,
        'cycle_info': cycle_info,
        'parameters_used': {
            'compression_ratio': compression_ratio,
            'expansion_n': expansion_n,
            'compression_n': compression_n,
...
def calculate_compression_work(pressure: np.ndarray, volume_ratio: np.ndarray, suction_pressure: float) -> float:
    """Calculate approximate compression work from P-V data"""
    try:
        # Simple integration approximation
        work = np.trapz(pressure, volume_ratio)
        return abs(work)
    except:
        return 0.0

def calculate_expansion_work(pressure: np.ndarray, volume_ratio: np.ndarray, discharge_pressure: float) -> float:
    """Calculate approximate expansion work from P-V data"""
    try:
        # Find expansion portion (approximately second half of cycle)
        mid_point = len(pressure) // 2
        expansion_pressure = pressure[mid_point:]
        expansion_volume = volume_ratio[mid_point:]
        work = np.trapz(expansion_pressure, expansion_volume)
        return abs(work)
    except:
        return 0.0

def estimate_indicated_power(pressure: np.ndarray, volume_ratio: np.ndarray, bore: float, stroke: float) -> float:
    """Estimate indicated power if cylinder dimensions are available"""
    if bore is None or stroke is None:
        return 0.0
    
    try:
        # Calculate swept volume in cubic inches
        swept_volume = np.pi * (bore / 2) ** 2 * stroke
        
        # Net work per cycle (simplified)
        net_work = np.trapz(pressure, volume_ratio * swept_volume)
        
        # Convert to horsepower (very approximate)
        # This is a simplified calculation for demonstration
        indicated_hp = abs(net_work) / 33000  # ft-lbs/min to HP conversion factor
        return indicated_hp
    except:
        return 0.0

def estimate_volumetric_efficiency(compression_ratio: float, clearance_ratio: float) -> float:
    """Estimate theoretical volumetric efficiency"""
    try:
        # Simplified volumetric efficiency calculation
        # ηv = 1 - clearance_ratio * (compression_ratio^(1/n) - 1)
        # Using approximate value for n
        n = 1.3  # Typical polytropic exponent for air
        efficiency = 1 - clearance_ratio * ((compression_ratio ** (1/n)) - 1)
        return max(0.0, min(1.0, efficiency))  # Clamp between 0 and 1
    except:
        return 0.85  # Default reasonable value

def extract_thermodynamic_parameters_from_xml(source_xml_content: str, levels_xml_content: str, cylinder_index: int) -> Dict[str, float]:
    """
    Extract real thermodynamic parameters from XML files for theoretical calculations
    
    Args:
        source_xml_content: Content of Source XML file
        levels_xml_content: Content of Levels XML file  
        cylinder_index: Cylinder index (1-based)
    
    Returns:
        Dictionary of thermodynamic parameters
    """
    try:
        source_root = ET.fromstring(source_xml_content)
        levels_root = ET.fromstring(levels_xml_content)
        
        # Helper function to find XML values (reuse existing function)
        def find_xml_value(root, worksheet_name, row_name, col_idx, occurrence=1):
            NS = {'ss': 'urn:schemas-microsoft-com:office:spreadsheet'}
            try:
                worksheet = next((ws for ws in root.findall('.//ss:Worksheet', NS) 
                                if ws.attrib.get('{urn:schemas-microsoft-com:office:spreadsheet}Name') == worksheet_name), None)
                if not worksheet:
                    return "N/A"
                ...
                for row in rows:
                    ...
                    if cells:
                        ...
                        if first_cell_data is not None and first_cell_data.text == row_name:
                            occurrence_count += 1
                            if occurrence_count == occurrence:
                                if col_idx < len(cells):
                                    value_node = cells[col_idx].find('ss:Data', NS)
                                    return value_node.text if value_node is not None else "N/A"
                return "N/A"
            except Exception:
                return "N/A"
        
        # Extract parameters for the specific cylinder
        col_idx = cylinder_index + 1  # XML uses 1-based indexing offset
        
        # Get compression ratio (use first occurrence for CE, second for HE)
        compression_ratio = find_xml_value(source_root, 'Source', 'COMPRESSION RATIO', col_idx, occurrence=1)
        
        # Get polytropic exponents
        expansion_n = find_xml_value(source_root, 'Source', 'EXPANSION N', col_idx, occurrence=1)
        compression_n = find_xml_value(source_root, 'Source', 'COMPRESSION N', col_idx, occurrence=1)
        
        # Get pressure values
        suction_pressure = find_xml_value(levels_root, 'Levels', 'SUCTION PRESSURE GAUGE', 2)
        discharge_pressure = find_xml_value(levels_root, 'Levels', 'DISCHARGE PRESSURE GAUGE', 2)
        
        # Get cylinder dimensions
        bore = find_xml_value(source_root, 'Source', 'COMPRESSOR CYLINDER BORE', col_idx)
        stroke = find_xml_value(source_root, 'Source', 'CALCULATED STROKE', col_idx)
        
        # Convert KPA to PSI for pressures
        def convert_kpa_to_psi(kpa_str):
            if kpa_str == "N/A" or not kpa_str:
                return None
            try:
                return float(kpa_str) * 0.145038
            except (ValueError, TypeError):
                return None
        
        # Convert string values to floats with error handling
        ...
        suction_psi = convert_kpa_to_psi(suction_pressure)
        discharge_psi = convert_kpa_to_psi(discharge_pressure)
        
        parameters = {
            'compression_ratio': safe_float(compression_ratio, 2.2),  # Default reasonable value
            'expansion_n': safe_float(expansion_n, 1.3),
            'compression_n': safe_float(compression_n, 1.35),
            'suction_pressure': suction_psi if suction_psi else 690.0,  # Default from data
            'discharge_pressure': discharge_psi if discharge_psi else 1550.0,  # Default from data
            'bore': safe_float(bore),
            'stroke': safe_float(stroke),
        ...
        st.warning(f"Could not extract thermodynamic parameters: {e}. Using default values.")
        return {
...
def apply_enhanced_pressure_options_to_plot(fig, df, cylinder_config, pressure_options, files_content):
    """
    Enhanced version of pressure options that uses real thermodynamic calculations
    MAINTAINS ALL EXISTING FUNCTIONALITY while adding advanced theoretical curves
    """
    if not pressure_options['enable_pressure']:
        return fig
    ...
    # PRESERVE EXISTING CE PT TRACE FUNCTIONALITY
    ...
    # ENHANCED CE THEORETICAL with REAL CALCULATIONS
    if pressure_options['show_ce_theoretical']:
        try:
            st.sidebar.write("Debug: Calculating Real CE Theoretical...")
            
            # Extract real thermodynamic parameters from XML
            if files_content and 'source' in files_content and 'levels' in files_content:
                cylinder_index = int(cylinder_config.get('cylinder_name', 'Cylinder 1').split()[-1])
                ...
                if thermo_params['extraction_success']:
                    ...
                    # Display cycle information in sidebar
                    ...
                    fallback_theoretical_ce(df, colors, fig, st)
    ...
    # ENHANCED HE THEORETICAL with REAL CALCULATIONS  
    if pressure_options['show_he_theoretical']:
        try:
            st.sidebar.write("Debug: Calculating Real HE Theoretical...")
            ...
            if files_content and 'source' in files_content and 'levels' in files_content:
                ...
                if thermo_params['extraction_success']:
                    ...
                    fallback_theoretical_he(df, colors, fig, st)
    ...
    # PRESERVE ALL EXISTING HE PT TRACE FUNCTIONALITY
    if pressure_options['show_he_pt']:
    ...
    # PRESERVE ALL EXISTING ADDITIONAL PRESSURE TRACES
    additional_pressure_options = [
        ('show_ce_nozzle', 'CE Nozzle pressure'),
        ('show_he_nozzle', 'HE Nozzle pressure'),
        ('show_ce_terminal', 'CE Terminal pressure'),
        ('show_he_terminal', 'HE Terminal pressure')
    ...
    for option_key, display_name in additional_pressure_options:
        if pressure_options[option_key]:
            st.sidebar.info(f"{display_name} data not available in current dataset")
    
    return fig

def fallback_theoretical_ce(df, colors, fig, st):
    ...
    fig.add_trace(
        go.Scatter(
            x=df['Crank Angle'],
            y=theoretical_ce,
            name='CE Theoretical (Simple)',
            line=dict(color=colors['ce_theoretical'], width=2, dash='dash'),
            mode='lines'
        ),
        secondary_y=False
    )
    st.sidebar.info("Using simple CE theoretical calculation (fallback)")

def fallback_theoretical_he(df, colors, fig, st):
    ...
    fig.add_trace(
        go.Scatter(
            x=df['Crank Angle'],
            y=theoretical_he,
            name='HE Theoretical (Simple)',
            line=dict(color=colors['he_theoretical'], width=2, dash='dash'),
            mode='lines'
        ),
        secondary_y=False
    )
    st.sidebar.info("Using simple HE theoretical calculation (fallback)")

def process_pressure_by_period(df, pressure_curve, period_selection, rpm=600):
    """
    PRESERVE EXISTING period processing functionality exactly as is
    This function remains unchanged to maintain compatibility
    """
    import numpy as np
    ...
    try:
        if period_selection == "Median":
            ...
            processed_pressure = (max_env + min_env) / 2 + (max_env - min_env) * 0.3
            
        elif period_selection == "Inner Envelope":
            processed_pressure = pressure_data * (0.95 + 0.1 * np.sin(np.arange(len(pressure_data)) * 0.1))
            
        else:  # "All periods"
            processed_pressure = pressure_data
